const Result = require('../../system/metadata/result.model');
const Reason = require('../../system/metadata/reason.model');
const Action = require('./action.model');
const ActionChain = require('../eventActionChain/actionChain.model');
const EventActionChain = require('../eventActionChain/eventActionChain.model');
const BlockAutomation = require('../eventActionChain/blockAutomation.model');
...
class ActionConfigService {
  ...
  async createActionChain(body) {
    ...
    const item = await ActionChain.create({ ...body, id });
    await CacheService.bumpNamespaceVersion("configs:chains");
    return item;
  }
  ...
  async listBlockAutomations(queryParams) {
    ...
    return CacheService.withVersionedCache("configs:block_automations", queryParams, CACHE_TTL.LONG, async () => {
      const [items, totalItems] = await Promise.all([
  ...
  async createBlockAutomation(body) {
    ...
    const item = await BlockAutomation.create({ ...body, id });
    await CacheService.bumpNamespaceVersion("configs:block_automations");
    return item;
  }
