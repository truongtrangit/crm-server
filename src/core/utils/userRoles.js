...
function isWithinManagerScope(manager, payload) {
  ...
  if (Array.isArray(payload)) {
    ...
    targetGroupAliases = payload
      .flatMap((a) => a.groups?.map((g) => g.groupAlias || g) || [])
      .filter(Boolean);
  } else if (payload) {
  ...
  if (Array.isArray(manager.departments)) {
    const managerLeadDepts = manager.departments
      .filter((d) => d.role === 'lead')
      .map((d) => d.deptAlias);
    managerDeptAliases =
      managerLeadDepts.length > 0
        ? managerLeadDepts
...
function isOwnerOrAdmin(userOrRole) {
  ...
  return [USER_ROLE_VALUES.OWNER, USER_ROLE_VALUES.ADMIN].includes(
    roleStr.toUpperCase(),
  );
}

function hasExplicitModuleAccess(user, moduleIdPattern, action) {
  if (
    !user ||
  ...
  return user.moduleAccess.some((entry) => {
    if (!entry.isEnabled) return false;
    const matchesModule =
      entry.moduleId === moduleIdPattern ||
      entry.moduleId.startsWith(moduleIdPattern + '.') ||
      moduleIdPattern.startsWith(entry.moduleId + '.');
    if (!matchesModule) return false;
    return (
      entry.customPermissions === null ||
      (Array.isArray(entry.customPermissions) &&
        entry.customPermissions.includes(action))
    );
  });
}
