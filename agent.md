...
11. **Backend Controller & Service Convention**: 
    ...
    - Các hàm trong file controller KHÔNG dùng khối `try/catch` và tham số `next`. Do dự án sử dụng **Express 5**, hệ thống sẽ tự động bắt các Promise rejection và chuyển cho Global Error Handler. Các controller hiện có nếu dùng `try/catch` kết hợp `next(error)` cần được gỡ bỏ hoàn toàn để thống nhất convention và giúp code trong sáng hơn. (Trừ trường hợp Controller cần xử lý lỗi đặc thù như trả về dữ liệu mặc định hoặc gỡ cookies).
    - **Cách Export Service**: Định nghĩa Service dưới dạng Lớp (Class) và export thể hiện (instance) của lớp đó ở cuối file (`class MyService { async doLogic() {} } module.exports = new MyService();`). KHÔNG khởi tạo 1 object khổng lồ từ đầu file rồi nhét function vào.
    - **Cách Export Controller**: Định nghĩa Controller dưới dạng Lớp (Class) và export thể hiện (instance) của lớp đó ở cuối file (`class MyController { async doAction(req, res) {} } module.exports = new MyController();`). KHÔNG export các object hàm lẻ tẻ `exports.doAction = ...`.
...
17. **File Placement Convention (Routes & Constants)**:
    - **Routes**: Mọi file khai báo routing (VD: `*.routes.js`) **BẮT BUỘC** phải nằm tập trung trong cấu trúc thư mục `src/routes/` (có thể là `v1/` hoặc `external/v1/`). **TUYỆT ĐỐI KHÔNG** để file routing rải rác bên trong thư mục module con của `src/modules/`.
    - **Constants**: Mọi file định nghĩa hằng số nghiệp vụ, enum hoặc status (VD: `*.js`, `courseChallenge.js`) **BẮT BUỘC** phải được đặt tập trung tại `src/core/constants/`. **TUYỆT ĐỐI KHÔNG** tạo file logic constant (VD: `*.constants.js`) nằm bên trong cấu trúc `src/modules/`.
18. **No Magic Strings (Constant Extraction Rule)**:
    - **BẮT BUỘC** phải đưa tất cả các giá trị trạng thái (status), loại (type), hoặc các giá trị enum đang bị hard-code (magic string) vào định nghĩa hằng số (constants) và sử dụng biến hằng số đó ở mọi nơi để tránh sai sót chính tả.
    - Ví dụ: Thay vì dùng `enrollment.status === 'ACTIVE'`, hãy import và dùng `COURSE_ENROLLMENT_STATUS.ACTIVE` từ `src/core/constants/appData.js`.
